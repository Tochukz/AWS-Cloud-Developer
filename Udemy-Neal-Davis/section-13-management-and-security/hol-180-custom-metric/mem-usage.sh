#!/bin/bash
# Description: Send memory usage as a custom metric to AWS CloudWatch
...
aws cloudwatch put-metric-data --region eu-west-2 --namespace "Custom/Memory" --metric-name "MemUsage" --value "$MEMORY_USAGE" --unit "Percent" --dimensions "Name=InstanceId,Value=$INSTANCE_ID"
